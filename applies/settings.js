 ...
 const City = {
  ...
  const Resume = {
    findById: async (resumeId) => {
      try {
        const response = await axios.get(`http://localhost:3003/api/resumes/${resumeId}`);
        return response.data;
      } catch (error) {
        console.error('Error fetching user data:', error);
        throw error;
  ...
  const Vacancy = {
    findById: async (vacancyId) => {
      try {
        const response = await axios.get(`http://localhost:3004/api/vacancies/${vacancyId}`);
        return response.data;
      } catch (error) {
        console.error('Error fetching user data:', error);
        throw error;
  ...
  module.exports = { City, Country, User, EmploymentType, Company, Resume, Vacancy };